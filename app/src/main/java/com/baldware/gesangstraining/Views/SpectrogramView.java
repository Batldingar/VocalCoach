...
public class SpectrogramView extends View {
     ...
     */
    public SpectrogramView(Context _context, AttributeSet _attributeSet) {
    ...
    /**
     * Sets mNoteView which determines whether to show the whole spectrum
     * or just the estimated target frequency (intended note) and the actual frequency
     *
    ...
    /**
     * Returns mNoteView which determines whether to show the whole spectrum
     * or just the estimated target frequency (intended note) and the actual frequency
     *
     * @return mNoteView
     */
    public boolean getNoteView() {
        return mNoteView;
    }
    ...
    private void fetchAndRedraw() {
        ...
        mWidth = mMemoryHandler.getXSize();
        mHeight = mMemoryHandler.getYSize();
        ...
        int accessX = Math.max(0, (mWidth - getWidth()));

        // Get values
        for (int y = 0; y < mHeight; y++) {
            for (int x = accessX; x < mWidth; x++) {
                dataArray[x - accessX + (y * mWidth)] = mMemoryHandler.get(x)[mHeight - 1 - y];
            }
        }

        if (mNoteView) {
            for (int x = accessX; x < mWidth; x++) {
                float[] currentSpectrum = mMemoryHandler.get(x);

                int binIndex = 0;

                for (int y = 0; y < mHeight; y++) {
                    if (currentSpectrum[y] == 1f) {
                        binIndex = y;
                        break;
                    }
                }
                ...
                if (nearestNoteFrequency != -1f) {
                    ...
                    if (dataArray[x - accessX + ((mHeight - 1 - nearestNoteBin) * mWidth)] != 1f) {
                        dataArray[x - accessX + ((mHeight - 1 - nearestNoteBin) * mWidth)] = -1f;
                    }
                }
            }
        }
        ...
        for (int i = 0; i < rgbArray.length; i++) {
            ...
            // Apply color coding
            float amplitudeHueScaled = dataArray[i] * 360f;
            ...
            if (amplitudeHueScaled != 0) {
                if (mNoteView) {
                    if (dataArray[i] == 1f) {
                        rgba = Color.argb(255, 255, 0, 255);
                    } else if (dataArray[i] == -1f) {
                        rgba = Color.argb(255, 0, 0, 0);
                    } else {
                        rgba = Color.argb(255, 255, 255, 255);
                    }
                } else {
                    rgba = Color.HSVToColor(new float[]{amplitudeHueScaled + 180, Math.min(1, dataArray[i] * 5), 1f}); // for rgb set saturation to 1f
                }
            } else {
                rgba = Color.argb(255, 255, 255, 255);
            }
    ...
    private int getMeasurement(int _measureSpec, int _contentSize) {
        int specMode = View.MeasureSpec.getMode(_measureSpec);
        int specSize = View.MeasureSpec.getSize(_measureSpec);
        int resultSize = 0;
